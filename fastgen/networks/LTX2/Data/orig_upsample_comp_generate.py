"""
Batch video generation comparing `prompt` vs `upsampled_prompt` at 5s.
Output files:
  {stem}_prompt_5s.mp4
  {stem}_upsampled_5s.mp4
"""
...
# ── Generation settings ───────────────────────────────────────────────────────
SEED      = 42
HEIGHT    = 1088   # nearest multiple of 64 to 1080
WIDTH     = 1920
NUM_FRAMES = 121   # 5s @ 24fps: (8 × 15) + 1 = 121

# Prompt variants to compare: output_suffix -> JSON key in prompts.txt
PROMPT_VARIANTS = {
    "prompt_5s":    "prompt",
    "upsampled_5s": "upsampled_prompt",
}
...
def output_path_for(source_file: str, suffix: str, output_dir: str) -> str:
    """e.g. '000431.json' + 'prompt_5s' -> './outputs_comparison/000431_prompt_5s.mp4'"""
...
def main() -> None:
    ...
    num_chunks    = get_video_chunks_number(NUM_FRAMES, tiling_config)
    total         = len(prompts)
    failed        = []  # list of (source_file, suffix)
    ...
    for idx, entry in enumerate(prompts, 1):
        ...
        for suffix, json_key in PROMPT_VARIANTS.items():
            prompt_text = entry.get(json_key, "").strip()
            out_path    = output_path_for(source_file, suffix, OUTPUT_DIR)

            logger.info(f"  [{suffix}] Prompt ({json_key}): {prompt_text[:100]}...")
            ...
            if not prompt_text:
                logger.error(f"  [{suffix}] Key '{json_key}' missing or empty, skipping.")
            ...
            try:
                ...
                encode_video(
                    ...
                    video_chunks_number=num_chunks,
                )
    ...
    # Summary
    logger.info("=" * 60)
    total_variants = total * len(PROMPT_VARIANTS)
    logger.info(
